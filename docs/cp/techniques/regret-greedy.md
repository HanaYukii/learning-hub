---
title: 反悔貪心(pq + doubly linked list)
tags: [反悔貪心, 貪心]
...
## 要點 / 易錯
- **邊界用哨兵,別手寫分支**:兩端補 `-INF` 哨兵後,取到端點時合併值自然變成不可再選(端點沒有「兩側」可反悔);手寫 `L==-1/R==-1` 分支極易漏。環形則無邊界,直接把 linked list 接成環。
