...
- **`std::format` 用 `{}` 佔位,type-safe,不用像 `printf` 手動對 `%d/%f`**:`#include <format>`,C++20;格式字串為常數時**格式錯誤在編譯期報錯**(P2216,回溯入 C++20);`{0}{1}{0}` 位置索引可重複引用同一參數;`std::print/println` 是 **C++23**(`<print>`)。
