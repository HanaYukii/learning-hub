...
```
題意 = **2–4 句,讀完能直接開始想題**:「給定 X(input 是什麼)。規則/操作…。**求/輸出**:Y(output 是什麼)」——input 與 output 都要明說,關鍵約束/範圍必寫。作法 1–2 行,**2000 分選手已懂的 common sense 跳過**(不解釋前綴和/Kadane/Dijkstra 是什麼),字數留給非顯然的關鍵步。
**沒收的題不寫進 digest 頁面**(篩選照做;exclusion 清單與理由只放 PR body / 回報訊息)。
**收錄(甜蜜區,兩端思考,難度本身不是排除理由):**
- CF 場:各難度都收(含 A/B),只要題型**主流、可遷移、學得起來**;簡單題給一句關鍵觀察即可。
- **ABC 場:前段不收——原則上從 E 起,且 CF-equivalent ≳ ~1700 才逐題收**(D 以下僅分數特別高才破例);略過的在 header 引言一行帶過(如「A–E 熱身略過;只收 F/G」)。
- **丟**:偏門 gimmick(ad-hoc / 一次性 / 吃靈光,之後幾乎用不到)。
- 難度 **~2700 為上限**;超過的**只在經典模板工具**(HLD、ODT、線段樹分治、可回滾DSU、分治NTT、BSGS、WHT…)時破例,否則不收。
